...
if button:
    ...
    if debug_mode == False:
        ...
        for percent_complete,i in enumerate(text_output.index):
            ...
            # 
            text_output.loc[i,'EXPNO 產品名'] = str(EXPNO對應表.loc[j,'產品名'])
            text_output.loc[i,'EXPNO 開狀人'] = str(EXPNO對應表.loc[j,'開狀人'])
            text_output.loc[i,'EXPNO 受益人'] = str(EXPNO對應表.loc[j,'受益人'])
            text_output.loc[i,'EXPNO 開狀銀行'] = str(EXPNO對應表.loc[j,'開狀銀行'])
            #
            text_output.loc[i,'EXPNO 產品名分數'] = get_jaccard_sim(str(產品),str(EXPNO對應表.loc[j,'產品名']))
            text_output.loc[i,'EXPNO 開狀人分數'] = get_jaccard_sim(str(開狀人),str(EXPNO對應表.loc[j,'開狀人']))
            text_output.loc[i,'EXPNO 受益人分數'] = get_jaccard_sim(str(受益人),str(EXPNO對應表.loc[j,'受益人']))
            text_output.loc[i,'EXPNO 開狀銀行分數'] = get_jaccard_sim(str(開狀銀行),str(EXPNO對應表.loc[j,'開狀銀行']))
    #==================================================================================================
